import {state,color,duration} from './data.js?v=3';
import {treemap,histogram} from './charts.js?v=3';
...
export function initStory(host) {
  ...
  step(0);
  return {destroy:cleanupWith(()=>{alive=false;mobileQuery.removeEventListener('change',placeVisual);observer.disconnect();ro.disconnect();tree.destroy();cancelAnimationFrame(frame);window.removeEventListener('scroll',schedule);window.removeEventListener('resize',schedule);s.selectAll('button').on('.story',null);points.selectAll('*').interrupt();})};
}
